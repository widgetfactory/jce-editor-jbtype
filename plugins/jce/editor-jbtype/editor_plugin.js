    ...
    tinymce.create('tinymce.plugins.JBType', {
        init: function(ed, url) {
            ...
            ed.onInit.add(function() {
                ...
                tinymce.each(boxes, function(name) {
                ...
                tinymce.each(colours, function(name) {
                    ed.formatter.register(name, {
                        inline: 'span',
                        classes: name + ' mceItemTypography'
                    });
                });
                tinymce.each(discs, function(name) {
                    ed.formatter.register(name, {
                        inline: 'span',
                        classes: name + ' mceItemTypography'
                    });
                });
                tinymce.each(iconic, function(name) {
                    ed.formatter.register(name, {
                        inline: 'span',
                        classes: name + ' mceItemTypography'
                    });
        ...
        createControl: function(n, cm) {
            ...
            switch (n) {
                case 'jbtype':
                    ...
                    tinymce.each(iconic, function(k) {
                        list.add('jbtype.' + k, k, {
                            'class': 'jb_icons ' + k,
                            icon: k
                        });
                    ...
                    tinymce.each(colours, function(k) {
                        list.add('jbtype.' + k, k, {
                            'class': k
                        });
                    });
        ...
        addType: function(v) {
            ed = this.editor, dom = ed.dom, s = ed.selection, n = s.getNode();
            switch (v) {
                case 'jb_blackbox':
                case 'jb_greenbox':
                case 'jb_bluebox':
